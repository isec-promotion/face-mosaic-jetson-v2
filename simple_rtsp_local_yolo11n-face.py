...
RTSP → nvurisrcbin(HWデコード/動的pad) → queue → nvvideoconvert
    → capsfilter(NV12@NVMM) → nvstreammux → nvinfer(YOLO11n-face検出)
...
- YOLO11n-face専用：顔のみを検出してモザイク処理

# 基本的な使用方法 
python3 simple_rtsp_local_yolo11n-face.py "rtsp://USER:PASS@CAMERA_IP:554/Streaming/Channels/101"

# 設定ファイルを明示的に指定 
python3 simple_rtsp_local_yolo11n-face.py "rtsp://USER:PASS@CAMERA_IP:554/Streaming/Channels/101" --infer-config ./config_infer_primary_face_yolo11n-face.txt

# 解像度とフレームレートを指定 
python3 simple_rtsp_local_yolo11n-face.py "rtsp://USER:PASS@CAMERA_IP:554/Streaming/Channels/101" --width 1920 --height 1080 --fps 30

# TCP接続を使用する場合（UDPで接続できない場合） 
python3 simple_rtsp_local_yolo11n-face.py "rtsp://USER:PASS@CAMERA_IP:554/Streaming/Channels/101" --tcp
"""
import argparse
import logging
import signal
...
# YOLO11n-faceモデルのクラスID（顔は常にクラスID 0）
FACE_CLASS_ID = 0

def parse_args():
    p = argparse.ArgumentParser(description="RTSP→YOLO11n-face検出→顔黒塗り→ローカル表示（DeepStream）")
    p.add_argument("rtsp_url", help="rtsp://user:pass@ip:554/Streaming/Channels/101")
    p.add_argument("--infer-config", default="./config_infer_primary_face_yolo11n-face.txt",
                   help="nvinfer の設定ファイル（YOLO11n-face検出）")
...
def build_pipeline(args: argparse.Namespace) -> Gst.Pipeline:
    ...
    # Primary Inference（YOLO11n-face検出）
    ...
    LOG.info("パイプライン構築完了（nvurisrcbin→YOLO11n-face→黒塗り→ローカル表示）")
    return pipe
